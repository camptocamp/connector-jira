...
class JiraBackend(models.Model):
    ...
    def _get_app_descriptor(self):
        ...
        return {
            ...
            "lifecycle": {
                "installed": f"{base_url}/jira/{bcknd_id}/installed",
                "uninstalled": f"{base_url}/jira/{bcknd_id}/uninstalled",
                "enabled": f"{base_url}/jira/{bcknd_id}/enabled",
                "disabled": f"{base_url}/jira/{bcknd_id}/disabled",
            },
            "modules": {
                "webhooks": [
                    {
                        "event": "jira:issue_created",
                        "url": f"{base_url}/connector_jira/{bcknd_id}/webhooks/issue",
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "url": f"{base_url}/connector_jira/{bcknd_id}/webhooks/worklog",
                    },
                    {
    ...
    def _install_app(self, payload):
        """Update the backend with proper settings after Jira app installation
    ...
    def _uninstall_app(self, payload):
        """Update the backend with proper settings after Jira app uninstallation"""
        self.ensure_one()
    ...
    def _enable_app(self, payload):
        """Update the backend with proper settings after Jira app enablement"""
        self.ensure_one()
    ...
    def _disable_app(self, payload):
        """Update the backend with proper settings after Jira app disablement"""
        self.ensure_one()
        query = "SELECT id from jira_backend WHERE id = %s FOR UPDATE"
        self.env.cr.execute(query, (self.id,))
        values = self._prepare_backend_values(payload)
